...
口号式的"零依赖"没有意义，必须能自动检查。本脚本静态扫描 ``arkdps``
下的所有 import，确认它们**只来自标准库**。

这不是洁癖：``arkdps`` 的本体是一个**公式引擎**，依赖越少，
"为什么算出来是这个数"就越容易复核。而且 clone 下来直接能跑，
不需要先配环境——这对一个拿来读代码的项目很重要。
...
def _configure_output_encoding() -> None:
    ...
    Windows 上标准输出被重定向时的默认编码是 GBK，而本脚本会输出中文
    与 ✔，不重配会直接抛 ``UnicodeEncodeError``——CI 的 Windows 任务
    正是这么挂的。
    """
